...
const TaskCard = ({ task }) => {
  ...
  const onUpdate = (status, id) => {
    axios
      .put(
        `https://task-manager-server-pi-self.vercel.app/updateTask/${status}/${id}`
      )
      .then((res) => {
        refetch();
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap justify-between items-center">
        ...
        <button className="cursor-pointer" onClick={showTheMOdal}>
          <FaArrowRight className="mt-2"></FaArrowRight>
        </button>
      </div>
      {showModal && (
        <TaskDetailsModal
...
const TaskDetailsModal = ({ taskDetails, onClose, onUpdate }) => {
  const { task_title, task_description, priority, deadline, status } =
    taskDetails;
          ...
          <div className="p-6">
            ...
            <div className="flex justify-start flex-wrap gap-4">
              {status === "pending" && (
                <button
                  onClick={() => {
                    onUpdate("ongoing", taskDetails?._id);
                  }}
              ...
              {status === "pending" || status === "ongoing" ? (
                <button
                  onClick={() => {
                    onUpdate("completed", taskDetails?._id);
                  }}
